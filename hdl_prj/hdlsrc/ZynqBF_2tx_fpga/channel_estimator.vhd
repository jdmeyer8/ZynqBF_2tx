-- -------------------------------------------------------------
-- 
-- File Name: hdl_prj\hdlsrc\ZynqBF_2tx_fpga\channel_estimator.vhd
...
ARCHITECTURE rtl OF channel_estimator IS

  -- Component Declarations
  COMPONENT goldSequences
    PORT( clk                             :   IN    std_logic;
          reset                           :   IN    std_logic;
          enb                             :   IN    std_logic;
  ...
  FOR ALL : goldSequences
    USE ENTITY work.goldSequences(rtl);

  FOR ALL : gs_selector
    USE ENTITY work.gs_selector(rtl);

  FOR ALL : peakdetect_ch1
    USE ENTITY work.peakdetect_ch1(rtl);

  FOR ALL : ram_rd_counter
    USE ENTITY work.ram_rd_counter(rtl);

  FOR ALL : ram_counter
    USE ENTITY work.ram_counter(rtl);

  FOR ALL : peakdetect_ch2
    USE ENTITY work.peakdetect_ch2(rtl);

  FOR ALL : state_machine
    USE ENTITY work.state_machine(rtl);

  FOR ALL : in_fifo
    USE ENTITY work.in_fifo(rtl);

  FOR ALL : rx_bram
    USE ENTITY work.rx_bram(rtl);

  FOR ALL : ch_est
    USE ENTITY work.ch_est(rtl);
...
BEGIN
  ...
  u_peakdetect_ch1 : peakdetect_ch1
    PORT MAP( clk => clk,
              reset => reset,
              enb => enb,
  ...
  u_ram_rd_counter : ram_rd_counter
    PORT MAP( clk => clk,
              reset => reset,
              enb => enb,
  ...
  u_peakdetect_ch2 : peakdetect_ch2
    PORT MAP( clk => clk,
              reset => reset,
              enb => enb,
              rst => Logical_Operator3_out1,
